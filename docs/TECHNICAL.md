...
Plan's pipeline originally followed the CSV pattern above for its financial data too (two QuickBooks reports, both converted to CSV first). It was rewritten the same day it was built, once Matt's actual refresh cadence turned out to be a single recurring report — **Sales by Customer Detail, run for All Dates** — that QuickBooks emails him every Monday at 8:00 AM CST; he moves the attachment into a Google Drive folder (`_Rolling Financials`) and archives the previous file. Reading a real copy of that file directly showed two things worth knowing for any future rework in this suite:
