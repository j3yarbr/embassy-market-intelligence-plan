...
**Real gotcha, found 2026-08-27, fixed at the root 2026-08-28**: `Get-Content $path | ConvertFrom-Csv` looks safe but isn't — `Get-Content` splits the file into lines on every raw newline *before* any CSV-quote-awareness applies, including a newline sitting inside a quoted field. Sage's `GeneralNotes`/`ContactNotes` fields trigger this (QuickBooks' own multi-line memos used to as well, before Plan's main data source moved off CSV entirely — see below). `Clean-AcctRep` in `build_plan_data.ps1` was an earlier band-aid that just filtered out the resulting garbage values ("TRUE", stray sentence fragments) rather than fixing the misalignment itself. The real fix: `Read-CsvSkippingHeaderLines` reads the file with `Get-Content -Raw` (one string, real newlines intact), splits off just the header lines by count (safe — headers never contain embedded newlines), and feeds the rest through `ConvertFrom-Csv` as one unbroken string so its own quote-aware parser handles the rest correctly. **Apply this pattern to any future CSV parsing in this suite** — the bug is in the `Get-Content | ConvertFrom-Csv` combination itself, not specific to any one file.

This pipeline is **not automated or live** — rerun by hand whenever Matt pulls a fresh export. Each build script currently expects its source file(s) already exported to a scratch folder (update the `$scratch` variable at the top before rerunning) — not yet a single one-command pipeline from `.xlsx` to deployed `data.json`.

## Plan's data source: one report read directly via Excel COM, not CSV (rebuilt 2026-08-28)

Plan's pipeline originally followed the CSV pattern above for its financial data too (two QuickBooks reports, both converted to CSV first). It was rewritten the same day it was built, once Matt's actual refresh cadence turned out to be a single recurring report — **Sales by Customer Detail, run for All Dates** — that QuickBooks emails him automatically (originally weekly, switched to daily 2026-09-09); he moves the attachment into a Google Drive folder (`_Rolling Financials`) and archives the previous file. Reading a real copy of that file directly showed two things worth knowing for any future rework in this suite:

1. **One report can replace two** if it's granular enough. This one is line-item level (one row per product line on an invoice/credit-memo, not one row per invoice/credit-memo, let alone one row per customer) — grouping its rows by (customer, invoice number) and filtering to Income-type accounts reproduces both the old per-invoice detail *and* the old audited per-customer total, validated to match the old two-report pipeline's own output almost exactly. Don't assume a report needs replacing with another report of the same shape — check whether a richer single source already covers the need.
2. **Read it directly via Excel COM (`UsedRange.Value2`, a bulk 2D array read), not through a CSV export.** A report with a free-text column (this one has `Description`) can carry the same embedded-newline hazard documented above; reading the `.xlsx` directly sidesteps the whole class of bug rather than needing the CSV fix reapplied to a new file shape. `build_plan_data.ps1`'s Section 1 is the reference implementation of this pattern — `Value2` returns date-formatted cells as **strings** (`"03/17/2026"`, not OLE Automation doubles) in this environment, confirmed by inspecting raw cell types directly rather than assuming — parse accordingly (`ParseExact` with `"MM/dd/yyyy"`, not `[datetime]::FromOADate`).
...
## One-off PDF reports: headless Edge print-to-pdf (no working Python)

For a one-time analysis delivered as a printable one-pager (e.g. Plan's Specials lift analysis, see `PLAN.md`) rather than a live site page, the reusable pattern in this environment — again, no working Python (see above) — is:

1. Build the report as a self-contained HTML/CSS artifact fragment, with a `@media print { @page { size: letter landscape; margin: ...; } ... }` block that aggressively shrinks font sizes/padding/gaps to fit the intended page count. `-webkit-print-color-adjust: exact !important;` is required to preserve background colors and badge fills in the print output — without it, Chromium-based print strips most non-text color.
2. Serve it locally — a headless browser can't reach an authenticated Artifacts URL directly, so wrap the fragment in a minimal `<!doctype html><html><head>...</head><body>...</body></html>` shell and serve it from the same PowerShell `HttpListener` static server used for `file://`-avoidance testing (see below).
3. Print it: `& "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" --headless --disable-gpu --no-pdf-header-footer --print-to-pdf="<output>.pdf" "http://localhost:<port>/<file>.html"`. **Always pass the `--print-to-pdf` path as an absolute path** — a relative path silently produced a stale/failed result in practice. Don't append a cache-busting query string to the served URL; the local `HttpListener` server in this repo resolves the path literally and 404s on one, which fails the print silently (exit code 2, no PDF written) rather than erroring loudly — if a regenerated PDF looks byte-identical to the previous one despite a real source edit, verify the print actually ran (check the file's write timestamp/size changed) before assuming the CSS change had no effect.
4. Verify the output with the `Read` tool's native PDF support (page count, text content, visual layout) rather than assuming — it renders each page as both extracted text and an image, enough to confirm a one-page fit and catch any overflow without needing a PDF library.
...
## Known gotcha: a literal `</script>` inside a JS string closes the *outer* script tag

Any inline `<script>` block that builds an HTML string containing `<script>...</script>` (e.g. Plan's per-month report export, which opens a self-printing report in a new tab) will break the page it's *generated from*, not just the generated page — the browser's HTML parser tokenizes `<script>`/`</script>` before any JS parsing happens, so it closes the outer script tag the instant it sees that literal substring anywhere in the source, even embedded inside a JS string literal. Everything after that point in the file silently stops being treated as script and the truncated script throws `SyntaxError: Invalid or unexpected token` — with no indication of *where*, since the actual break is wherever the closing tag happened to fall, not near any real syntax error. Found while building Plan's month-report export (2026-09-09): diagnosed by fetching the raw page source and testing each top-level function's body individually with `new Function(...)` until the bad one isolated. Fix: split the literal so it never appears contiguous in the HTML source — `"<scr" + "ipt>...` and `"</scr" + "ipt>"` — rather than escaping characters within the string, which doesn't help since the HTML parser runs before JS string escaping is ever interpreted. **Apply this any time generated JS needs to emit a `<script>` tag as a string, in this suite or otherwise.**
