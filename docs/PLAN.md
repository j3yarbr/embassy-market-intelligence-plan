...
## Why the panel asks for two separate files

The pipeline (PowerShell script and this panel both) genuinely needs two different pieces of QuickBooks data that don't live in the same report — this isn't an arbitrary UI choice:
- **Income by Customer Summary** — the audited revenue total per customer, the number that actually ties to accounting. Gross invoice amounts don't: they include sales tax, and one real invoice checked directly was $1,005.99 gross vs. $931.68 Income, an ~8% gap.
- **Transaction List by Customer** — invoice-level rows (date, amount, invoice number). Used to distribute the audited Income total across months proportionally, count orders, find the last invoice date, and — in Append mode — dedup against what's already loaded by invoice number.

Neither report alone carries both pieces, so both are currently required to compute a correct, month-by-month, dedupable number.

**Open question, not yet resolved (flagged 2026-08-28):** Matt's actual cadence is a single QuickBooks report scheduled to drop into a Google Drive folder called `_Rolling Financials` every Monday — not two separate manual exports. Whether that scheduled report is actually one of these two (Transaction List or Income Summary alone), a different combined report (e.g. a "Sales by Customer Detail" export, a format that shows up elsewhere among Matt's own QuickBooks pulls), or something else entirely determines whether the two-file panel as built actually fits that real workflow, or needs to change to work from a single file instead. Needs Matt to confirm what the scheduled report actually contains before deciding anything here.

**Overwrite vs. Append for a routine weekly drop:** once the file question above is settled, Append is very likely the right standing mode either way — invoice-number dedup means even a report with an overlapping/wide date range can't double-count revenue for weeks already loaded. Overwrite is for a full reset (first load, or recovering from a bad append), not a weekly habit.

Still manual today either way: whoever's covering has to open Plan, upload from that Drive folder, download the output file(s), and hand them back for a commit. A same-day session raised a more automatic alternative worth real consideration later — Claude Code watching the `_Rolling Financials` folder directly and running the existing PowerShell pipeline + git commit itself when a new file lands, using authority already granted this session (Drive read access, git push authority) rather than a browser-stored GitHub credential. Not built; needs the file-format question above answered first. See `BACKLOG.md`.
