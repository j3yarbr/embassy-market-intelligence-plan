...
2. **Month-to-Month Revenue chart** — a Monthly/Daily toggle in the chart's own header (added 2026-09-10, relocated there from the table below it same day — see below) drives both the chart and the collapsible "Monthly figures" table together. In Monthly mode: single-hue bar chart with a YoY reference tick + month-over-month labels, plus a table with a YoY column and a click-to-expand detail view per month — companies this month vs. the same month last year, top-5 sellers each year, a win-back list (real prior-year buyers with zero revenue this month, ranked by dollar size), and an "Export one-pager" button that opens a print-ready report for that month in a new tab (added 2026-09-09 — see its own section below). In Daily mode: the chart shows one bar per day, chronological (the shape of revenue over time); the table becomes a Daily Pareto — every day in the selected range ranked by revenue, same Pareto concept as the Client Sales table's Cumulative Total, applied to days (concentration, not shape).
...
## Monthly/Daily toggle: chart + Pareto table (added 2026-09-10, toggle relocated same day)

Matt asked to add "a day by day pareto along with the month to month... same concept just daily view," first built as a toggle inside the collapsed Monthly figures table panel. He then clarified: "I really just wanted the image above updated to a daily toggle... need the chart more than the table" — the toggle needed to live on the always-visible **Month-to-Month Revenue chart**, not buried inside a collapsed section below it.

**Toggle** (`#chartGranularityToggle`, in the chart's own header, same chip-row pattern as the CRM table's Group: Industry/Company toggle): "Monthly" (default) vs. "Daily". **One toggle now drives both the chart and the Monthly figures table together** — `state.revenueGranularity` (renamed from `state.monthTableGranularity` when the toggle moved) — so the two views can never disagree about which mode is showing.

**Two different reads of the same day-level data, on purpose**:
- **The chart** (`renderDailyChart`) stays **chronological** — one bar per day, in date order, same visual language as the existing monthly bar chart (`renderChartOrDaily` dispatches between `renderChart` and `renderDailyChart`, swapping the section title/note/legend and the table panel's title along with it). Shows the day-to-day *shape* — spikes, lulls, trend.
- **The table** (`renderDailyPareto`, in the collapsible "Daily figures — Pareto (table view)" panel) stays **ranked by revenue descending** with % of Total / Cumulative % — the actual Pareto concept Matt named, showing revenue *concentration*. "Same concept" = the Client Sales table's existing Cumulative Total pattern, applied to days instead of companies.

Daily bars/rows both drop the monthly chart's YoY reference tick and month-over-month labels — a single day compared to the same day last year, or to the day before, is too noisy to be a useful read (a $50 Tuesday after a $1 Monday is a meaningless "+4900%"). The classic Pareto *chart* shape (bars + a plotted cumulative-% line on a second axis) was deliberately not built either way — a second y-axis is the #1 dataviz anti-pattern in this suite's own skill guidance, and the existing CRM Pareto view is already a plain table, not a chart.

**Daily computation, shared**: `data.json`'s `clients[].monthly` is only month-granularity, so day-level revenue needs real per-invoice dates — from `invoices.json`, **not** fetched on a normal page load (lazy raw artifact). `getDailyRows(months, filtered)` is the one shared fetch-and-aggregate helper (`loadInvoicesOnce()` caches the fetch) used by the chart, the Pareto table, and `exportDailyCsv` — filtered to the currently filtered companies and the selected month range's day boundaries, returned in ascending date order; the Pareto table and CSV export re-sort it descending by amount, the chart uses the ascending order directly.

**Verified, not just built**: the Daily Pareto total reconciles exactly with the "Revenue in Range" KPI tile, both unfiltered (175 days, cumulative reaching 100% at the same dollar figure as the KPI) and filtered down to a single Industry chip (1 day, $721, matching the filtered KPI exactly). Toggling back to Monthly restores the original chart/table/legend/title exactly.

**Scope kept deliberately narrow**: no per-day expand/drill-down (companies, SKUs, win-back) on either the chart or the table.
