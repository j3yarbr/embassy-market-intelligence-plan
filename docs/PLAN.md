...
2. **Month-to-Month Revenue chart** — single-hue bar chart, hover tooltips, plus a collapsible "Monthly figures" table with a Monthly/Daily (Pareto) toggle (added 2026-09-10 — see below). In Monthly mode: a YoY column and a click-to-expand detail view per month — companies this month vs. the same month last year, top-5 sellers each year, a win-back list (real prior-year buyers with zero revenue this month, ranked by dollar size), and an "Export one-pager" button that opens a print-ready report for that month in a new tab (added 2026-09-09 — see its own section below). In Daily mode: every day in the selected range ranked by revenue, same Pareto concept as the Client Sales table's Cumulative Total, applied to days.
...
## Monthly figures: Daily Pareto toggle (added 2026-09-10)

Matt asked to add "a day by day pareto along with the month to month... same concept just daily view," as a toggle inside the Monthly figures panel rather than a separate section. "Same concept" here means the Client Sales (CRM) table's existing Pareto pattern — % of Total and Cumulative Total, ranked by revenue descending — applied to calendar days instead of companies, to see whether revenue in the selected range is concentrated on a handful of big days or spread evenly.

**Toggle** (`#monthGranularityToggle`, same chip-row pattern as the CRM table's Group: Industry/Company toggle): "Monthly" (default, unchanged existing behavior) vs. "Daily (Pareto)". Both write into the same `#monthTable` element — `renderMonthOrDailyTable()` dispatches to the existing `renderMonthTable()` or the new `renderDailyPareto()` and swaps the `<thead>` accordingly, so there's only ever one table on the page, not two.

**Daily computation**: `data.json`'s `clients[].monthly` is only month-granularity, so day-level revenue needs real per-invoice dates — from `invoices.json`, which is **not** fetched on a normal page load (kept as a lazy raw artifact, same as it always was for the now-removed append-dedup use case). `loadInvoicesOnce()` fetches and caches it only the first time Daily is selected, so a routine visit to Plan never pays for it. Each invoice's `date`/`amount` is filtered to the currently filtered companies (respecting Industry/Rep chips, same as everywhere else on the page) and the currently selected month range's day boundaries, summed per calendar day, then sorted descending by revenue with running % of Total / Cumulative % columns — plain table, no dual-axis chart (the classic Pareto chart shape — bars + a plotted cumulative-% line on a second axis — was deliberately not built; a second y-axis is the #1 dataviz anti-pattern in this suite's own skill guidance, and the existing CRM Pareto view is already a plain table, not a chart, so this stays consistent with that precedent).

**Verified, not just built**: the Daily Pareto total reconciles exactly with the "Revenue in Range" KPI tile, both unfiltered (175 days, cumulative reaching 100% at the same dollar figure as the KPI) and filtered down to a single Industry chip (1 day, $721, matching the filtered KPI exactly).

**Scope kept deliberately narrow**: no per-day expand/drill-down (companies, SKUs, win-back), no same-day-last-year comparison — daily YoY would be noisy at single-day granularity and wasn't part of the ask. Export CSV branches to a day-level CSV (`exportDailyCsv`) when Daily is selected, same columns as the on-page table.
